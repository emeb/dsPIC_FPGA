...
{
    //return(!SD_CD);
	return TRUE;		// EMEB - No CD on my board so always TRUE
}//end MediaDetect
...
{
    // Turn off the card
//    SD_CD_TRIS = INPUT;            //Card Detect - input EMEB - no CD on my board
    SD_CS = 1;                     //Initialize Chip Select line
    SD_CS_TRIS = OUTPUT;            //Card Select - output
//    SD_WE_TRIS = INPUT;            //Write Protect - input EMEB - no WE on my board
}
...
{
    //return(SD_WE);
	return FALSE;		// EMEB - no WE on my board, always return FALSe
}
